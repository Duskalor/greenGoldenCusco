...
export function Navbar({
	...
	const navMessages: Record<Lang, string> = {
		es: "Hola, quiero reservar un tour",
		en: "Hi, I want to book a tour",
		pt: "Olá, quero reservar um tour",
	};
		...
		<nav
			className={`fixed top-0 left-0 right-0 z-50 transition-all duration-400 ${
				transparent
					...
					: "bg-[rgba(15,23,12,0.96)] backdrop-blur-xl border-b border-gold/15 py-[10px]"
			}`}
		>
			<div className="max-w-[1200px] mx-auto px-6 flex items-center justify-between">
				<Link href="/" className="flex items-center gap-2.5 no-underline">
					<span className="text-[28px] font-bold font-display text-gold">
						GG
					</span>
					<div className="h-5 w-px bg-gold/35" />
					<span className="text-[10px] font-sans text-white/65 tracking-[3px] uppercase">
						Green Golden Cusco
					</span>
				</Link>
				...
				<div className="hidden md:flex gap-6 items-center">
					...
					<Button
						as="a"
						href={getWhatsAppUrl(navMessages[lang])}
						target="_blank"
						rel="noreferrer"
						size="sm"
					>
				...
				<div className="flex md:hidden items-center gap-2.5">
					...
					<button
						...
						className="bg-transparent border-0 text-gold text-2xl cursor-pointer p-1"
					>
