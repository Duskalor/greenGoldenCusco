...
export function ToursClient() {
	...
	const filtered = tours.filter((tour: Tour) => {
		if (filter === "fullday")
			return (
				!tour.duration.includes("D") &&
				!tour.duration.includes("Día") &&
				!tour.duration.includes("dia")
			);
		if (filter === "multiday")
			return tour.duration.includes("D") || tour.duration.includes("N");
		return true;
	});
