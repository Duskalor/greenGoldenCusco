...
export function MediaClient() {
  ...
  const categories: string[] = ['all', ...(Array.from(new Set(gallery.map((g: any) => String(g.cat)))) as string[])]
  const filtered = filter === 'all' ? gallery : gallery.filter((g: any) => g.cat === filter)
      ...
      <Container className="py-16">
            ...
            {categories.map((cat) => (
              <button key={cat} onClick={() => setFilter(cat)}
                className={`px-4 py-1.5 rounded-full text-xs font-sans font-semibold tracking-wide transition-all duration-300 border ${
                  filter === cat ? 'bg-gold text-dark border-gold' : 'bg-transparent text-white/50 border-white/12 hover:border-gold/30 hover:text-white'
                }`}>
                {cat === 'all' ? mp.all : cat}
              </button>
            ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {filtered.map((item: any, i: number) => (
            <Animate key={i} delay={i * 0.05}>
              <Card className="aspect-square flex items-center justify-center relative overflow-hidden group cursor-pointer">
        ...
        <Animate delay={0.2}>
          <Card className="mt-12 p-10 text-center">
            <h3 className="font-sans font-bold text-white mb-3">{mp.videoTitle}</h3>
